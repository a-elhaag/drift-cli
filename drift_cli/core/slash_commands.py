...
class SlashCommandRegistry:
    ...
    COMMANDS = {
        ...
        "/git": SlashCommand(
            ...
            query_template=(
                "Analyze the current git repository status and suggest the next logical "
                "action. Check for uncommitted changes, unpushed commits, branches, etc."
            ),
            category="git",
            requires_git=True,
            icon="🔀",
        ...
        "/commit": SlashCommand(
            ...
            query_template=(
                "Generate a concise, conventional commit message based on the staged "
                "changes. Follow conventional commit format (feat:, fix:, docs:, etc.). "
        ...
        "/status": SlashCommand(
            ...
            query_template=(
                "Run git status and explain what each section means. Suggest next actions "
                "based on the status."
            ),
            category="git",
            requires_git=True,
        ...
        "/push": SlashCommand(
            ...
            query_template=(
                "Check if there are commits to push. Verify the remote branch exists. "
                "Suggest safe push command with --force-with-lease if needed."
            ),
            category="git",
            requires_git=True,
            icon="⬆️",
        ...
        "/pull": SlashCommand(
            ...
            query_template=(
                "Pull latest changes safely. Check for local changes first. Suggest stash "
                "if needed. Use --rebase if appropriate."
            ),
            category="git",
            requires_git=True,
            icon="⬇️",
        ...
        "/find": SlashCommand(
            ...
            query_template=(
                "Find files or content in the current directory. Use modern tools like "
                "fd, rg, or fallback to find/grep. Safe read-only operations only."
            ),
            category="files",
            icon="🔍",
        ),
        ...
        "/large": SlashCommand(
            ...
            query_template=(
                "Find and list large files (>10MB) in current directory and "
                "subdirectories. Show sizes in human-readable format."
            ),
            category="files",
            icon="📦",
        ),
        "/tree": SlashCommand(
            ...
            query_template=(
                "Display directory tree structure. Use tree command if available, "
                "otherwise use find with formatting. Limit depth to 3 levels."
            ),
            category="files",
            icon="🌳",
        ),
        ...
        "/fix": SlashCommand(
            ...
            query_template=(
                "Analyze recent command errors and environment issues. Suggest specific "
                "fixes based on error messages, common patterns, and project context."
            ),
            category="system",
            icon="🔧",
        ),
        "/clean": SlashCommand(
            ...
            query_template=(
                "Identify and safely remove build artifacts, cache files, and temporary "
                "files. Suggest commands specific to the project type (node_modules, "
                "__pycache__, .next, target, etc.). Ask for confirmation."
            ),
            category="system",
            icon="🧹",
        ),
        "/deps": SlashCommand(
            ...
            query_template=(
                "Check project dependencies based on project type. For Python: check "
                "pip/requirements. For Node: check package.json. Suggest update commands "
                "if needed."
            ),
            category="system",
            icon="📚",
        ...
        "/port": SlashCommand(
            ...
            query_template=(
                "Find what process is using a specific port. If no port specified, show "
                "common development ports (3000, 8080, 5000, etc.) and their status."
            ),
            category="system",
            icon="🔌",
        ),
        ...
        "/test": SlashCommand(
            ...
            query_template=(
                "Detect project type and run appropriate test command. For Python: pytest. "
                "For Node: npm test. For Go: go test. Show verbose output."
            ),
            category="workflow",
            icon="🧪",
        ),
        "/build": SlashCommand(
            ...
            query_template=(
                "Detect project type and run appropriate build command. For Python: "
                "pip install -e . For Node: npm build. For Go: go build."
            ),
            category="workflow",
            icon="🔨",
        ),
        "/dev": SlashCommand(
            ...
            query_template=(
                "Detect project type and start development server. For Python: "
                "uvicorn/flask. For Node: npm run dev. For static: python -m "
                "http.server."
            ),
            category="workflow",
            icon="🚀",
        ...
        "/lint": SlashCommand(
            ...
            query_template=(
                "Detect project type and run appropriate linter. For Python: "
                "ruff/black/flake8. For Node: eslint. For Go: gofmt. Show and optionally "
                "fix issues."
            ),
            category="workflow",
            icon="✨",
        ...
        "/tips": SlashCommand(
            ...
            query_template=(
                "Based on my command history and patterns, suggest workflow improvements, "
                "aliases, or better ways to accomplish common tasks."
            ),
            category="meta",
            icon="💡",
        ),
...
class SlashCommandHandler:
    ...
    def check_requirements(self, command: SlashCommand) -> tuple[bool, Optional[str]]:
        ...
        Returns:
            (ok, error_message)
        """
        if command.requires_git and not self._is_git_repo():

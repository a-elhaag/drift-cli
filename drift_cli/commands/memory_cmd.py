...
def show_memory():
    ...
    prefs_table.add_row(
        "Risk Tolerance",
        "Comfortable with high-risk" if memory.preferences.comfortable_with_high_risk 
        else "Prefers safer approaches"
    )
    prefs_table.add_row(
        "Dry-run Preference",
        "Enabled" if memory.preferences.prefers_dry_run else "Disabled"
    )
    prefs_table.add_row(
        "Explanation Style",
...
def show_insights():
    ...
    context_panel = Panel(
        context,
        title="[cyan]Context Sent to AI[/cyan]",
        border_style="cyan"
    )
...
def export_memory(
    output: str = typer.Argument(..., help="Output file path (JSON format)")
):
...
def import_memory(
    ...
    merge: bool = typer.Option(False, "--merge", "-m", help="Merge with existing preferences instead of replacing"),
):
    ...
    try:
        ...
        if merge:
            ...
            memory.preferences.comfortable_with_high_risk = (
                memory.preferences.comfortable_with_high_risk or 
                prefs_data["comfortable_with_high_risk"]
            )
        else:
            # Replace entirely
            ...
            memory.preferences.prefers_verbose_explanations = prefs_data["prefers_verbose_explanations"]
        ...
        console.print(f"[green]✓ Memory imported successfully![/green]")
        console.print(f"[cyan]Imported {len(prefs_data['favorite_tools'])} tools, "
